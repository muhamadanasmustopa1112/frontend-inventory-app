...
export async function GET(req: NextRequest) {
  try {
    ...
    // Staff / admin gudang → paksa warehouse_id ke gudangnya sendiri
    if (user.warehouse_id !== null) {
      query.set("warehouse_id", String(user.warehouse_id))
    } else {
      // Admin pusat → boleh pakai warehouse_id dari query (kalau dikirim)
      if (requestedWarehouseId) {
        query.set("warehouse_id", requestedWarehouseId)
      }
    }

    const backendUrl = `${API_URL}/stock-out?${query.toString()}`
    ...
    if (!res.ok) {
      return NextResponse.json(
        {
          message: "Failed to fetch stock-out from backend",
          status: res.status,
    ...
    return NextResponse.json(
      {
        message: "Unexpected error when fetching stock-out",
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!res.ok) {
      return NextResponse.json(
        {
          message: data?.message || "Failed to create stock-out",
          errors: data?.errors,
        },
        { status: res.status }
      )
    ...
    return NextResponse.json(
      {
        message: "Unexpected error when creating stock-out",
        error: error?.message ?? String(error),
      },
      { status: 500 }
    )
